...
class model:
    def __init__(self, M, b, dir_path=False):
        ...
        # input matrix and vector data
            ...
            self.Mm =  np.where(M<0.0, M,  0.0)
            self.Mp =  np.where(M>=0.0, M,  0.0)
    ...
    def solve(self, x0=None, err=10**(-6), max_itr=100):
        ...
        x_list = [x_pre]
        obj_value = lambda x : (x@self.M@x) + self.b@x
...
#             alpha = self.Calc_StepSize(x_pre, y)
    ...
    def SubProb(self, x_pre):
        ...
        model.setObjective((x_pre@(self.Mm - self.Mp.T + self.Mm.T - self.Mp) + self.b)@x \
                            + x@(self.Mp + self.Mp.T)@x)
    ...
    def SubProb_R(self, x_pre):
        model = gp.Model()

        x = model.addMVar(self.N, lb=0.0)
        model.setObjective((x_pre@(self.M + self.M.T) + self.b)@x)
        model.addConstr(self.M@x >= - self.b)
    ...
    def Calc_StepSize(self, x_pre, y):
        ...
        #     return x_new@self.M@x_new +self.b@x_new
        

        # K = 1000
        # alpha = np.argmin(np.array([obj_alpha(k*(1/K), x_pre, y) for k in range(K)]))*(1/K)
